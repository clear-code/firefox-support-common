...
* GPUのサンドボックス化をWindowsで有効化。（Firefox 110）
  * X-Mouse Button Control（XMBC） バージョン2.20より古いバージョンを使用している場合、マウスのホイールスクロールが動作しなくなる可能性があります。（XMBCバージョン2.20以降への更新が推奨されます。）
...
* Windows 10およびWindows 11において、インストーラの実行なしにタスクバーにFirefoxをピン留めできるようになった。（Firefox 103）
  ...
  * この変更は可逆的で、INIファイルの`TaskbarShortcut`を`false`にすることで従来の動作に戻せます。
