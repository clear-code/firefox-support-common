...
* Total Cookie Protectionを初期状態で有効化し、すべてのサードパーティーCookieをそれぞれのオリジン用のストレージ領域に保存するようになり、トラッキング防止がより強力になった。（Firefox 103）
  * この変更は可逆的で、`network.cookie.cookieBehavior`を`4`に、`browser.contentblocking.features.strict`を  
    `tp,tpPrivate,cookieBehavior5,cookieBehaviorPBM5,cm,fp,stp,lvl2,rp,rpTop,ocsp,qps`  
    にすることで従来の動作に戻せます。  
