...
* アドレスバーからWeb検索を実行した際の検索結果のページを表示中は、検索サービスのURLではなく、入力した検索語句をアドレスバーに表示できるようになった。（Firefox 113）
  ...
  * `browser.urlbar.showSearchTerms.featureGate`=`true`の時にのみ機能及び設定UIが有効化されます。
    ![](esr115/addressbar-show-search-terms-setting.png)
  * `browser.urlbar.showSearchTerms.enabled`=`false`とすることで従来の動作に戻ります。
...
* Windowsにおいて、[アンチウィルスソフトなどのサードパーティ製モジュール自身が、クラッシュ防止などのためにモジュール側の判断で自身をFirefoxに読み込ませなくできるようになった](https://support.mozilla.org/kb/identify-problems-third-party-modules-firefox-windows)。（Firefox 110）
* [複数のセキュリティデバイスを追加していた際の、about:policiesでのエラーを解消。](https://bugzil.la/1807008)（Firefox 110）
* about:third-partyでのサードパーティ製モジュールの読み込み停止操作を行えなくするポリシーである[`DisableThirdPartyModuleBlocking`](https://github.com/mozilla/policy-templates/blob/master/README.md#disablethirdpartymoduleblocking)に対応。（Firefox 110）
...
* ポリシー設定`EnableTrackingProtection`が、新しい`EmailTracking`オプションに対応。（Firefox 112）
* ESR版においてのみ、ポリシー設定`Preferences`で`xpinstall.signatures.required`を制御できるようになった。（Firefox 112）
* プライベートウィンドウにおいて、サードパーティCookieやトラッカーのストレージ領域へのデータ保存のより効果的なブロックのために、ユーザーイベントを保護するようになった。（Firefox 113）
...
* 非推奨のU2F JavaScript APIが初期状態で無効化された。（U2FプロトコルはWebAuthn APIで引き続き使用可能）（Firefox 112）
  * 隠し設定`security.webauth.u2f`で機能を再度有効化可能。
