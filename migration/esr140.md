...
-->
* 新規設定項目の `network.http.basic_http_auth.enabled` を `false` にすることにより、HTTP Basic認証を全面的に無効化できるようになった。（Firefox 136）
...
* ユーザーの操作で新しいウィンドウが開かれたときなど、いくつかの場面で暗黙的にサードパーティコンテンツにストレージアクセスの許可を与えていた動作が無効化され、コンテンツがStorage Access APIによる明示的な許可を得なくてはならなくなった。（Firefox 132）
  * スクリプトでポップアップウィンドウを開く作りのWebページについて、設計が古いと影響を承ける可能性があります。
