...
* 「他の端末からのタブ」を、従来のサイドバーからのほか、タブの一覧表示メニューからも表示できるようになった（Firefoxアカウントのログイン時のみ有効）。（Firefox 133）  
  ![（画像：タブの一覧表示→他の端末からのタブ）](./esr140/133_tab_overview.png) ■日本語表示のスクショに差し替える
...
* サイドバーメニューを拡充し、メイン画面を表示させたまま各種ツール[^muliple-tools]へワンクリックで素早くアクセスできるようになった。（Firefox 136）
  ![（画像：サイドバーのスクリーンショット）](./esr140/136_sidebar.png)  ■「新しいサイドバー」メニューのみ有効化したものに差し替え
...
* タブ（1つまたは複数選択）上で右クリック→「タブを解放」選択でタブをアンロードできる。メモリーおよびCPU使用の削減に貢献。（Firefox Nightly 140）
...
* 直感的な検索キーワード指定。`@bookmarks`、`@tabs`、`@history`、`@actions`といった説明的なキーワードを使い、アドレスバーから利用できる検索モードへアクセスできるようになった。（Firefox 137）  
  ![（画像：利用できる検索モードの一覧表示例）](./esr140/fx137_search_keywords.png) ■日本語の表示結果に差し替える 
...
* `SkipTermsOfUse`[^SkipTermsOfUse] ポリシーを追加し、Firefoxの新たな利用規約を非表示にできるようになった。（Firefox 138）
  * `SkipTermsOfUse`ポリシーの使用により、利用規約を承諾したこと、また、本ブラウザーにアクセスできる人物すべてに代わって利用規約を承諾する権限を有することを示す。
  * ■5/21記：利用規約の変更点を把握しておく
