...
* 「設定」→「Firefox Labs」[^Firefox-Labs]ページにより、実験的な機能を簡単に試せるようになった。（Firefox 130）
  * この機能は `UserMessaging` ポリシーの `FirefoxLabs` で無効化できます。（Firefox 130）
<!--法人利用への影響は軽微と思われるため省略
...
* ウェブページ翻訳[^translation]で、日本語への翻訳に対応。（Firefox 135）
...
* サイドバーメニューを拡充し、メイン画面を表示させたまま各種ツール[^muliple-tools]へワンクリックで素早くアクセスできるようになった。（Firefox 136）
  ...
  * この機能は初期状態で無効で、「設定」→「一般」→「ブラウザーレイアウト」→「サイドバーを表示」、またはMCDで `sidebar.revamp` （`true`＝有効、`fakse`＝無効）を変更することで制御できます。
* 「垂直タブ」をサポート。設定により、タブの一覧をサイドバー内に縦方向に並べて表示できるようになった。（Firefox 136）  
  ![（画像：垂直タブのスクリーンショット）](./esr140/136_vertical_tabs_expanded.png) 
  * この機能は初期状態で無効で、「設定」→「一般」→「ブラウザーレイアウト」→「垂直タブ」、またはMCDで `sidebar.verticalTabs` （`true`＝有効、`fakse`＝無効）を変更することで制御できます。
<!--法人利用への影響は軽微と思われるため省略
...
-->
* タブグループ[^tab-groups]を導入。グループは別のタブ上へドラッグすることで作成でき、グループ単位で名前付け、色分け、保存が可能。閉じたグループを再度開くこともできる。（Firefox 137で順次導入、Firefox 138で完全有効化）
...
* タブ（1つまたは複数選択）上で右クリック→「Unload Tab」選択でタブをアンロードできる。メモリーおよびCPU使用の削減に貢献。（Firefox Nightly 140）
...
* `DisableFirefoxStudies` ポリシー有効時にNimbus（Firefoxの実験ツール）も無効化されるようにした。（Firefox 129）
...
* 位置情報などへのアクセスを一時的に許可する機能を追加。許可は1時間後またはタブを閉じるまで有効となる。（Firefox 131）  
  ...
  * この機能による一時的な許可の有効期限は、MCDで `privacy.temporary_permission_expire_time_ms` を用いて制御できます。
...
* いわゆるmixed contentのページで、Webサイトのアイコン（favicon）がHTTPSで取得できないとブロックするようになった。（Firefox 132）
  * この動作は、ドキュメント中に含まれる画像の扱いと併せて、MCDで `security.mixed_content.upgrade_display_content.image` を `false` にすることでセキュリティの低下と引き換えに機能を無効化できます。
...
* 証明書の透明性が必須となり、証明書を信頼する前にWebサーバーに対してその証明書が公開されているという十分な証拠を求めるようになった（MozillaのRoot CA Program[^mozilla-root-ca-program]に基づいてFirefoxの証明書データベースに初期状態で登録されているすべての認証局が対象）。（Firefox 135）
  * この挙動は、`Preferences` ポリシー[^Preferences]で `security.pki.certificate_transparency.mode` を `0`（完全無効）または `1`（統計情報の収集のmい）とすることで、従来の挙動に戻せます。（Firefox 133）
...
* `SkipTermsOfUse`[^SkipTermsOfUse] ポリシーを追加し、新たな利用規約を非表示にできるようになった。（Firefox 138）
  * **`SkipTermsOfUse`ポリシーの使用により、利用規約を承諾したこと、また、本ブラウザーにアクセスできる人物すべてに代わって利用規約を承諾する権限を有することを示す。** ■原文スタイルを踏襲しボールド
* アドオンはWebページにリソースを注入できるが、これが悪意あるサイトにとってフィンガープリンティングのベクター（媒介者）として役立つ可能性がある。ユーザープライバシー保護を強化するため、アドオンにより注入されたリソースに対して追跡防止となる分離機能を実験中。（Firefox 140Nightly）
* 「強化型トラッキング防止機能」が「厳格」のときの挙動が変化した。
  * サードパーティーCookieをブロックするようになった。（Firefox 132）
  * Bounce Tracking（中間サイトを介したトラッキング）を防御する機能を追加。リダイレクト動作に基づき検知し、定期的にCookieとサイトデータを消去するようになった。（Firefox 133）
  * 強化型トラッキング防止向け「SmartBlock」[^Smartblock-Embeds]機能の追加。「強化型トラッキング防止機能」の「厳格」モードおよびプライベートブラウジングモードで、特定のソーシャルメディアの埋め込みをブロックしなくなる。除外対象となる埋め込みタイプは順次追加される予定。（Firefox 136）
...
* タブを切り替えたとき、再生中の動画を自動的にピクチャーインピクチャーで開けるようになった。（初期設定では無効）
...
* ユーザー識別の拒否を明示的に通知する機能について、古い仕様である「Do Not Track」ではなく、グローバル・プライバシー・コントロール（GPC）[^global-privacy-control]に基づいた「ウェブサイトにユーザーデータの販売や共有の拒否を通知する」設定を使用するようになった。（Firefox 135）
