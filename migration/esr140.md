...
* アドオンはWebページにリソースを注入できるが、これが悪意あるサイトにとってフィンガープリンティングのベクター（媒介者）の役割を果たす可能性もある。ユーザープライバシー保護を強化するため、アドオンにより注入されたリソースを分離し、追跡防止を図る機能を実験中。（Firefox 140Nightly）
* 「強化型トラッキング防止機能」が「厳格」のときの挙動が変化した。
  ...
  * 「SmartBlock」[^Smartblock-Embeds]機能の追加。「強化型トラッキング防止機能」の「厳格」モードおよびプライベートブラウジングモードで、特定のソーシャルメディアの埋め込みをブロックしなくなる。除外対象となる埋め込みタイプは順次追加される予定。（Firefox 136）
